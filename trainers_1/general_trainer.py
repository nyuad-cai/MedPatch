...
class GeneralTrainer(Trainer):
    def __init__(self, 
        ...
        run = wandb.init(project=f'Fusion_General_{self.args.H_mode}_{self.args.task}', config=args)
        self.epoch = 0 
        self.start_epoch = 0
        ...
        self.transformer_layer1 = CustomTransformerLayer(input_dim=384 * len(self.args.modalities.split('-')), model_dim=384, nhead=4, num_layers=1).to(self.device)
