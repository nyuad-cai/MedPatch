...
class WeightedEnsembleLateTrainer(Trainer):
    def __init__(self, 
        train_dl, 
        val_dl, 
        args,
        ...
        super(WeightedEnsembleLateTrainer, self).__init__(args)
        ...
        self.early_final_classifier = MLPClassifier(input_dim=384, output_dim=self.args.num_classes).to(self.device)
        self.joint_final_classifier = MLPClassifier(input_dim=384, output_dim=self.args.num_classes).to(self.device)
        ...
        self.weights = nn.Parameter(torch.rand(3, device=self.device))
        ...
        if self.args.load_early:
            ...
            self.early_transformer_layer.load_state_dict(checkpoint['transformer_layer1_state_dict'])
            self.early_final_classifier.load_state_dict(checkpoint['final_classifier_state_dict'])
            print("early loaded")
        ...
        if self.args.load_joint:
            ...
            self.joint_transformer_layer.load_state_dict(checkpoint['transformer_layer1_state_dict'])
            self.joint_final_classifier.load_state_dict(checkpoint['final_classifier_state_dict'])
            print("joint loaded")
        ...
        for param in self.rr_classifier.parameters():
            param.requires_grad = False
        for param in self.early_transformer_layer.parameters():
            param.requires_grad = False
        for param in self.joint_transformer_layer.parameters():
            param.requires_grad = False
        for param in self.early_final_classifier.parameters():
            param.requires_grad = False
        for param in self.joint_final_classifier.parameters():
            param.requires_grad = False
        ...
        all_params = (
                ...
                list(self.early_final_classifier.parameters()) +
                list(self.joint_final_classifier.parameters()) +
    ...
    def save_fusion_checkpoint(self):
        ...
        checkpoint = {
            ...
            'early_final_classifier_state_dict': self.early_final_classifier.state_dict(),
            'joint_final_classifier_state_dict': self.joint_final_classifier.state_dict(),
    ...
    def load_fusion_checkpoint(self, checkpoint_path):
        ...
        self.early_final_classifier.load_state_dict(checkpoint['early_final_classifier_state_dict'])
        self.joint_final_classifier.load_state_dict(checkpoint['joint_final_classifier_state_dict'])
        self.ehr_classifier.load_state_dict(checkpoint['ehr_classifier_state_dict'])
        self.cxr_classifier.load_state_dict(checkpoint['cxr_classifier_state_dict'])
        self.rr_classifier.load_state_dict(checkpoint['rr_classifier_state_dict'])
        self.dn_classifier.load_state_dict(checkpoint['dn_classifier_state_dict'])
    ...
    def set_train_mode(self):
        ...
        self.early_final_classifier.train()
        self.joint_final_classifier.train()
    ...
    def set_eval_mode(self):
        ...
        self.early_final_classifier.eval()
        self.joint_final_classifier.eval()
    ...
    def train_epoch(self):
        ...
        for i, (x, img, dn, rr, y_ehr, y_cxr, seq_lengths, pairs) in enumerate (self.train_dl):
            ...
            if 'EHR' in self.args.modalities:
                ...
                late_v_ehr, cls_ehr = self.late_ehr_encoder(x)
            ...
            if 'CXR' in self.args.modalities:
                ...
                late_v_cxr, cls_cxr = self.late_cxr_encoder(img)
            ...
            if 'DN' in self.args.modalities:
                ...
                late_v_dn, cls_dn = self.late_dn_encoder(dn)
            ...
            if 'RR' in self.args.modalities:
                ...
                late_v_rr, cls_rr = self.late_rr_encoder(rr)
            ...
            y_fused_early = self.early_final_classifier(early_fused_vector[:, 0, :])
            y_fused_joint = self.joint_final_classifier(joint_fused_vector[:, 0, :])
            y_fused_late = torch.mean(torch.stack(y_preds, dim=0), dim=0)
            
            weights_normalized = self.weights / self.weights.sum()
            y_fused_stack = torch.stack([y_fused_early, y_fused_joint, y_fused_late], dim=0) 
            weighted_y_fused_stack = y_fused_stack * weights_normalized[:, None, None]
            y_fused_pred = weighted_y_fused_stack.sum(dim=0)
