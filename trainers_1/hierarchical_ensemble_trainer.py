...
class HierEnsembleFusionTrainer(Trainer):
    def __init__(self, 
        train_dl, 
        val_dl, 
        args,
        ...
        super(HierEnsembleFusionTrainer, self).__init__(args)
        ...
        self.transformer_layer1 = CustomTransformerLayer(input_dim=384 * len(self.args.modalities.split('-')) + 384, model_dim=384, nhead=4, num_layers=1).to(self.device)
    ...
    def train_epoch(self):
        ...
        for i, (x, img, dn, rr, y_ehr, y_cxr, seq_lengths, pairs) in enumerate(self.train_dl):
            ...
            # Concatenate joint vectors and token vector
            batch_size = x.size(0)
            ...
            joint_fused_vector = torch.cat((joint_fused_vector, token_vector_expanded), dim=1)
            joint_fused_vector = self.transformer_layer1(joint_fused_vector)

            # Concatenate early vectors and joint fused vector
            early_fused_vector = torch.cat(early_vectors, dim=1)
            early_fused_vector = torch.cat((early_fused_vector, joint_fused_vector), dim=1)
            early_fused_vector = self.transformer_layer2(early_fused_vector)

            # Concatenate late vectors and early fused vector
            late_fused_vector = torch.cat(late_vectors, dim=1)
            late_fused_vector = torch.cat((late_fused_vector, early_fused_vector), dim=1)
            late_fused_vector = self.transformer_layer3(late_fused_vector)

            # Final classifier
            y_fused_pred = self.final_classifier(late_fused_vector[:, 0, :])
