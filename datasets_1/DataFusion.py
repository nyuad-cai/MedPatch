...
def loadmetadata(args, discharge_notes, radiology_reports, cxr_reports):
    ...
    if args.data_pairs == 'paired':
        ...
        if 'DN' in args.modalities:
            cxr_merged_icustays = cxr_merged_icustays.merge(discharge_notes[dsrr_columns], how='inner', on=dsrr_merge_columns)
        ...
        if 'RR' in args.modalities:
            cxr_merged_icustays = cxr_merged_icustays.merge(radiology_reports[dsrr_columns], how='inner', on=dsrr_merge_columns)
        ...
        print("Starting partial data pairing...")
        
        ehr_icustays = icu_stay_metadata[columns]
        print("ehr_icustays shape after column selection:", ehr_icustays.shape)

        ehr_icustays.intime = pd.to_datetime(ehr_icustays.intime)
        ehr_icustays.outtime = pd.to_datetime(ehr_icustays.outtime)
        
        cxr_ehr_merged_icustays = cxr_metadata.merge(icu_stay_metadata[columns], how='inner', on='subject_id')
        print("cxr_ehr_merged_icustays shape after merge:", cxr_ehr_merged_icustays.shape)
        
        cxr_ehr_merged_icustays.intime = pd.to_datetime(cxr_ehr_merged_icustays.intime)
        cxr_ehr_merged_icustays.outtime = pd.to_datetime(cxr_ehr_merged_icustays.outtime)
        
        cxr_ehr_merged_icustays['StudyTime'] = cxr_ehr_merged_icustays['StudyTime'].apply(lambda x: f'{int(float(x)):06}')
        cxr_ehr_merged_icustays['StudyDateTime'] = pd.to_datetime(cxr_ehr_merged_icustays['StudyDate'].astype(str) + ' ' + cxr_ehr_merged_icustays['StudyTime'].astype(str), format="%Y%m%d %H%M%S")
    
        print("Added StudyDateTime column. Shape remains:", cxr_ehr_merged_icustays.shape)
        
        cxr_ehr_merged_icustays['time_diff'] = cxr_ehr_merged_icustays.StudyDateTime - cxr_ehr_merged_icustays.intime
        cxr_ehr_merged_icustays['time_diff'] = cxr_ehr_merged_icustays['time_diff'].apply(lambda x: np.round(x.total_seconds() / 60 / 60, 3))

        print("Calculated time_diff column.")

        cxr_ehr_merged_icustays['full_stay_time'] = cxr_ehr_merged_icustays.outtime - cxr_ehr_merged_icustays.intime
        cxr_ehr_merged_icustays['full_stay_time'] = cxr_ehr_merged_icustays['full_stay_time'].apply(lambda x: np.round(x.total_seconds() / 60 / 60, 3))
        
        print("Calculated full_stay_time column.")
        
        dsrr_merge_columns = ['subject_id', 'hadm_id']
        dsrr_columns = ['subject_id', 'hadm_id','charttime', 'text']
        
        print("Merging discharge notes...")
        dn_ehr_merged_icustays = icu_stay_metadata[columns].merge(discharge_notes[dsrr_columns], how='left', on=dsrr_merge_columns)
        dn_ehr_merged_icustays.rename(columns={'text': 'discharge_text'}, inplace=True)
        dn_ehr_merged_icustays.rename(columns={'charttime': 'discharge_charttime'}, inplace=True)
        dn_ehr_merged_icustays['discharge_charttime'] = pd.to_datetime(dn_ehr_merged_icustays['discharge_charttime'])
        print("dn_ehr_merged_icustays shape:", dn_ehr_merged_icustays.shape)

        dn_ehr_merged_icustays.intime = pd.to_datetime(dn_ehr_merged_icustays.intime)
        dn_ehr_merged_icustays.outtime = pd.to_datetime(dn_ehr_merged_icustays.outtime)
        
        print("Merging radiology reports...")
        rr_ehr_merged_icustays = icu_stay_metadata[columns].merge(radiology_reports[dsrr_columns], how='inner', on=dsrr_merge_columns)
        rr_ehr_merged_icustays.rename(columns={'text': 'radiology_text'}, inplace=True)
        rr_ehr_merged_icustays.rename(columns={'charttime': 'radiology_charttime'}, inplace=True)
        rr_ehr_merged_icustays['radiology_charttime'] = pd.to_datetime(rr_ehr_merged_icustays['radiology_charttime'])
        print("rr_ehr_merged_icustays shape:", rr_ehr_merged_icustays.shape)
        
        rr_ehr_merged_icustays.intime = pd.to_datetime(rr_ehr_merged_icustays.intime)
        rr_ehr_merged_icustays.outtime = pd.to_datetime(rr_ehr_merged_icustays.outtime)
        
        if args.task == 'in-hospital-mortality':
            print("this is mortality")
            print("Processing in-hospital mortality task...")
            end_time = cxr_ehr_merged_icustays.intime + pd.DateOffset(hours=48)
            print("End time calculated.")
            
            cxr_ehr_merged_icustays = cxr_ehr_merged_icustays.loc[((cxr_ehr_merged_icustays.StudyDateTime >= cxr_ehr_merged_icustays.intime) & (cxr_ehr_merged_icustays.StudyDateTime <= end_time))]
            print("Filtered cxr_ehr_merged_icustays based on StudyDateTime:", cxr_ehr_merged_icustays.shape)
            cxr_ehr_merged_icustays = cxr_ehr_merged_icustays[cxr_ehr_merged_icustays['ViewPosition'] == 'AP']
            print("Filtered cxr_ehr_merged_icustays for ViewPosition 'AP':", cxr_ehr_merged_icustays.shape)
                
            rr_ehr_merged_icustays = rr_ehr_merged_icustays.loc[((rr_ehr_merged_icustays['radiology_charttime'] >= rr_ehr_merged_icustays.intime)&(rr_ehr_merged_icustays['radiology_charttime'] <= rr_ehr_merged_icustays.intime + pd.DateOffset(hours=48)))]
            print("Filtered rr_ehr_merged_icustays:", rr_ehr_merged_icustays.shape)

            group_columns = [col for col in rr_ehr_merged_icustays.columns if col not in ['radiology_text', 'radiology_charttime']]
            print("Grouping by columns:", group_columns)
            rr_ehr_merged_icustays['radiology_text'] = rr_ehr_merged_icustays['radiology_text'].fillna('').astype(str)
            for col in group_columns:
                if rr_ehr_merged_icustays[col].dtype == 'object':  # For categorical/text columns
                    rr_ehr_merged_icustays[col] = rr_ehr_merged_icustays[col].fillna('')
                elif 'datetime' in str(rr_ehr_merged_icustays[col].dtype):  # For datetime columns
                    rr_ehr_merged_icustays[col] = rr_ehr_merged_icustays[col].fillna(pd.Timestamp('1900-01-01'))
                else:  # For numeric columns
                    rr_ehr_merged_icustays[col] = rr_ehr_merged_icustays[col].fillna(0)
            rr_ehr_merged_icustays = rr_ehr_merged_icustays.groupby(group_columns, as_index=False).agg({'radiology_text': lambda texts: ' '.join(texts),'radiology_charttime': 'max'})
            print("Grouped rr_ehr_merged_icustays. New shape:", rr_ehr_merged_icustays.shape)

            final_merged = ehr_icustays.merge(
                cxr_ehr_merged_icustays, 
                how='left', 
                on=['subject_id', 'stay_id'], 
                suffixes=('', '_cxr')
            )
            print("Final merged with CXR data. Shape:", final_merged.shape)
            # Merge radiology reports onto final_merged with left join
            cxr_merged_icustays_AP = final_merged.merge(
                rr_ehr_merged_icustays[['subject_id', 'stay_id', 'radiology_text', 'radiology_charttime']], 
                how='left', 
                on=['subject_id', 'stay_id']
            )
            print("Final CXR and radiology merged shape:", cxr_merged_icustays_AP.shape)
            cxr_merged_icustays_AP['discharge_text']=None
        elif args.task == 'phenotyping':
            print("this is pheno")
            print("Processing pheno task...")
            end_time = cxr_ehr_merged_icustays.outtime
            print("End time calculated.")
            ...
            final_merged = dn_ehr_merged_icustays.merge(
                cxr_ehr_merged_icustays, 
                how='left', 
                on=['subject_id', 'stay_id'], 
                suffixes=('', '_cxr')
