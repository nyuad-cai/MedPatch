...
class EnsembleAverageTrainer(Trainer):
    def __init__(self, 
        train_dl, 
        val_dl, 
        args,
        ...
        super(EnsembleAverageTrainer, self).__init__(args)
        ...
        # Adjusted input_dim to 384 since we're averaging features
        self.early_transformer_layer = CustomTransformerLayer(input_dim=384, model_dim=384, nhead=4, num_layers=1).to(self.device)
        self.joint_transformer_layer = CustomTransformerLayer(input_dim=384, model_dim=384, nhead=4, num_layers=1).to(self.device)
        self.late_transformer_layer = CustomTransformerLayer(input_dim=384, model_dim=384, nhead=4, num_layers=1).to(self.device)
        self.final_transformer_layer = CustomTransformerLayer(input_dim=384, model_dim=384, nhead=4, num_layers=1).to(self.device)
    ...
    def train_epoch(self):
        ...
        for i, (x, img, dn, rr, y_ehr, y_cxr, seq_lengths, pairs) in enumerate (self.train_dl):
            ...
            dn_vectors = []
            cxr_vectors = []
            final_fused_vector = torch.tensor([]).to(self.device)
            ...
            if 'EHR' in self.args.modalities:
                ...
                ehr_vectors.append(early_v_ehr)
                ehr_vectors.append(joint_v_ehr)
                ehr_vectors.append(late_v_ehr)
                ehr_fused_vector = torch.stack(ehr_vectors, dim=0).mean(dim=0)
                final_fused_vector = torch.cat([final_fused_vector, ehr_fused_vector], dim=1)
            ...
            if 'CXR' in self.args.modalities:
                ...
                cxr_vectors.append(early_v_cxr)
                cxr_vectors.append(joint_v_cxr)
                cxr_vectors.append(late_v_cxr)
                cxr_fused_vector = torch.stack(cxr_vectors, dim=0).mean(dim=0)
                final_fused_vector = torch.cat([final_fused_vector,cxr_fused_vector], dim=1)
            ...
            if 'DN' in self.args.modalities:
                ...
                dn_vectors.append(early_v_dn)
                dn_vectors.append(joint_v_dn)
                dn_vectors.append(late_v_dn)
                dn_fused_vector = torch.stack(dn_vectors, dim=0).mean(dim=0)
                final_fused_vector = torch.cat([final_fused_vector, dn_fused_vector], dim=1)
            ...
            if 'RR' in self.args.modalities:
                ...
                rr_vectors.append(early_v_rr)
                rr_vectors.append(joint_v_rr)
                rr_vectors.append(late_v_rr)
                rr_fused_vector = torch.stack(rr_vectors, dim=0).mean(dim=0)
                final_fused_vector = torch.cat([final_fused_vector, rr_fused_vector], dim=1)
